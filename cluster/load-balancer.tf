...
resource "null_resource" "gloo" {
  ...
  provisioner "local-exec" {
    ...
    command = "bash ${path.module}/scripts/install-gloo.sh"
  }
}
...
resource "google_compute_region_backend_service" "default" {
  ...
  backend {
    ...
    group = "https://www.googleapis.com/compute/v1/projects/${var.project_name}/zones/${var.zone}/networkEndpointGroups/ingressgateway"
    capacity_scaler = 1
    balancing_mode = "RATE"
